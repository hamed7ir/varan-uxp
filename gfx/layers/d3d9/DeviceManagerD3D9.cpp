...
#include "gfxPrefs.h"
#include "mozilla/Sprintf.h"                    // Varan (M5 Lever D): SprintfLiteral for caps dump
#include "../../angle/src/common/VaranGpuLog.h" // Varan (M5 Lever D): %TEMP%\varan-gpu.log caps dump
...
DeviceManagerD3D9::VerifyCaps()
{
  ...
  // Varan (M5 Lever D): FL9_1 hardware (Tegra 3) tops out at a 2048 max texture, but the
  // historical gate demanded 4096 (needs FL9_3+ / desktop). Relax the minimum to 2048 on
  // ARM. Self-consistent: the screen is 1366x768 and the layer system tiles content to
  // mMaxTextureSize; WebGL already runs at 2048 on this device.
#if defined(_M_ARM)
  const UINT kVaranMinMaxTexture = 2048;
#else
  const UINT kVaranMinMaxTexture = 4096;
#endif

  // Varan (M5 Lever D): evaluate EVERY gate below (no early return) and, on ARM, log each
  // PASS/FAIL plus the actual cap value to %TEMP%\varan-gpu.log, so ONE device trip names
  // all shortfalls instead of only the first. VARAN_GATE takes each ORIGINAL failure
  // condition verbatim (zero inversion risk). Production verdict is unchanged: return
  // false iff any hard gate fails; device-dependent state is set only when all gates pass.
  bool varanOk = true;
#if defined(_M_ARM)
#define VARAN_GATE(label, failCond, val)                                     \
  do {                                                                       \
    bool _f = (failCond);                                                    \
    char _b[128];                                                            \
    SprintfLiteral(_b, "VARAN-GPU CAPS: %-22s %s val=0x%08x\n",              \
                   (label), _f ? "FAIL" : "PASS", (unsigned)(val));          \
    VaranGpuLog(_b);                                                         \
    if (_f) varanOk = false;                                                 \
  } while (0)
#else
#define VARAN_GATE(label, failCond, val)                                     \
  do { if (failCond) varanOk = false; } while (0)
#endif

  VARAN_GATE("TEXTUREVIDEOMEMORY",
             LACKS_CAP(caps.DevCaps, D3DDEVCAPS_TEXTUREVIDEOMEMORY),
             caps.DevCaps);
  VARAN_GATE("CULLNONE",
             LACKS_CAP(caps.PrimitiveMiscCaps, D3DPMISCCAPS_CULLNONE),
             caps.PrimitiveMiscCaps);
  VARAN_GATE("BLEND_SRC_DST",
             LACKS_CAP(caps.SrcBlendCaps, D3DPBLENDCAPS_ONE) ||
             LACKS_CAP(caps.SrcBlendCaps, D3DBLEND_SRCALPHA) ||
             LACKS_CAP(caps.DestBlendCaps, D3DPBLENDCAPS_INVSRCALPHA),
             caps.SrcBlendCaps);
  VARAN_GATE("SCISSORTEST",
             LACKS_CAP(caps.RasterCaps, D3DPRASTERCAPS_SCISSORTEST),
             caps.RasterCaps);
  VARAN_GATE("TEXCAPS_ALPHA_POW2",
             LACKS_CAP(caps.TextureCaps, D3DPTEXTURECAPS_ALPHA) ||
             ...
             caps.TextureCaps);
  VARAN_GATE("FILTER_LINEAR",
             LACKS_CAP(caps.TextureFilterCaps, D3DPTFILTERCAPS_MAGFLINEAR) ||
             LACKS_CAP(caps.TextureFilterCaps, D3DPTFILTERCAPS_MINFLINEAR),
             caps.TextureFilterCaps);
  VARAN_GATE("ADDRESS_CLAMP",
             LACKS_CAP(caps.TextureAddressCaps, D3DPTADDRESSCAPS_CLAMP),
             caps.TextureAddressCaps);
  VARAN_GATE("MAXTEXTURE_WIDTH",
             caps.MaxTextureWidth < kVaranMinMaxTexture,
             caps.MaxTextureWidth);
  VARAN_GATE("MAXTEXTURE_HEIGHT",
             caps.MaxTextureHeight < kVaranMinMaxTexture,
             caps.MaxTextureHeight);
  VARAN_GATE("PIXELSHADER_2_0",
             (caps.PixelShaderVersion & 0xffff) < 0x200,
             caps.PixelShaderVersion);
  VARAN_GATE("VERTEXSHADER_2_0",
